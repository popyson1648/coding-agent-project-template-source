...
公開テンプレートとして配布される内容は `coding-agent-project-template/` 以下にあります。
...
- **`scripts/verify.py`**：ローカル検証とCIで使う入口。

## 変更前に読む文書

変更の種類に合わせて、先に読む文書を選びます。
公開フローを変える場合は、`.project/structure.md` と `.project/release.md` を読みます。
検証やCIを変える場合は、`.project/verification.toml`、`scripts/verify.py`、`.github/workflows/ci.yml` を読みます。
テンプレート利用者向けの説明を変える場合は、`coding-agent-project-template/README.md` を読みます。
